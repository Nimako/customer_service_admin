...
class ClientController extends Controller
{
    ...
    public function UpdateClient(Request $request){
       ...
       if(empty($client)){
        ...
        $update_client = Client::where('id', $client->id)
                        ->update([
                          'FirstName'         => !empty($request->FirstName)?$request->FirstName:$client->FirstName,
                          'LastName'          => !empty($request->LastName)?$request->LastName:$client->LastName,
                          'Email'             => !empty($request->Email)?$request->Email:$client->Email,
                          'Region'            => !empty($request->Region)?$request->Region:$client->Region,
                          'CityTown'          => !empty($request->CityTown)?$request->CityTown:$client->CityTown,
                          'DateBirth'         => !empty($request->DateBirth)?$request->DateBirth:$client->DateBirth,
                          'Gender'            => !empty($request->Gender)?$request->Gender:$client->Gender,
                          'ProfilePicture'    => !empty($request->ProfilePicture)?$request->ProfilePicture:$client->ProfilePicture,
    ...
    public function AllClient(){
        ...
        return view('client/allClient',$data);
    }
    ...
    public function ClientDetail($id){
        ...
        $data['info'] = Client::where('sha1(id)',$id)->first();
        ...
        if(empty($client)){
            ...
            return redirect()->action([ClientController::class, 'AllClient']);
        } 
    }
    ...
    public  function SendOTP(Request $request){
        ...
        if(!empty($client->id)){ 
            $this->response = ["statusCode" => 200, "IsCustomer"=>"true",   "OTP"=>$OTP, "message" => "Phone number already exist verify OTP and login"];
        }else{
            $this->response = ["statusCode" => 500, "IsCustomer"=>"false", "OTP"=>$OTP, "message" => "Phone Number does not exist OTP sent Please to signup"];
        }
